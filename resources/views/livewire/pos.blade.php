...
<div class="{{ $isKioskDevice ? 'h-dvh w-screen overflow-hidden flex flex-col' : 'min-h-screen' }} bg-gray-50 dark:bg-gray-900" x-data="posCart()" x-init="
    ...
    <script>
        ...
        // full doc-type/head/body markup string — that string used to
        // contain its own literal closing body and html tags, and
        // something in the response pipeline (Livewire's own script/asset
        // injection scans rendered HTML for the real page's closing body
        // tag to insert into) matched THAT occurrence instead, splicing a
        // script tag into the middle of this one and breaking it —
        // everything after the split point then rendered as plain visible
        // text on the page instead of running as JS. Avoiding that literal
