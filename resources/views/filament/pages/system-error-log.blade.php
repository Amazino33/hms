    ...
    <div class="max-w-5xl mx-auto mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                Every real system error (not routine validation messages) — read straight from the server's log file, so it still shows up even if the database itself is the problem. Refreshes automatically.
            </p>
        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                @if ($expandedIndex === $i)
                    <div class="px-4 pb-4 border-t border-gray-100 dark:border-gray-700 pt-3">
                        ...
                        <pre class="text-xs bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-3 overflow-x-auto whitespace-pre-wrap text-gray-700 dark:text-gray-300">{{ $entry['trace'] ?? 'No trace recorded.' }}</pre>
                    </div>
                @endif
            </div>
        @empty
