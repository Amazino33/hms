...
it('adds to existing stock on re-import instead of silently overwriting it, and logs a transaction', function () {
    ...
    $warehouse = WareHouse::create(['name' => 'Main Bar', 'type' => 'consumer']);

    // First import: brand-new product, 10 units.
    $path1 = writeProductImportFixture('SKU-IMPORT-1', 10, 'Main Bar');
    Excel::import(new ProductImport(), $path1);
    unlink($path1);

    $product = Product::where('sku', 'SKU-IMPORT-1')->firstOrFail();
    expect((int) $product->inventory()->where('warehouse_id', $warehouse->id)->value('quantity'))->toBe(10);
    ...
    $path2 = writeProductImportFixture('SKU-IMPORT-1', 6, 'Main Bar');
    Excel::import(new ProductImport(), $path2);
    unlink($path2);

    expect((int) $product->inventory()->where('warehouse_id', $warehouse->id)->value('quantity'))->toBe(16);
